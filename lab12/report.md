...
def check(obj, ctx, type, size_strategy):
    ...
    if not is_top_level and f'{type} {obj.name}' not in prev
        and obj.pointer_level == 0 and obj.name and obj.fields is None:
...
# | Expr * Expr | Expr / Expr | SIZEOF ( VARNAME ) | - Expr | ( Expr )
NExpr |= NTerm
...
В рамках выполнения данной работы я получил навыки выполнения семантического
анализа.
